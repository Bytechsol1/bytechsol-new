...
const Blogdetail = () => {
  ...
  const initialData = getInitialBlogData();
  const initialPost =
    initialData?.slug === slug && initialData.blogPost ? initialData.blogPost : null;
  const initialRelated =
    initialData?.slug === slug && Array.isArray(initialData.relatedPosts)
  ...
  const applyPostContent = (rawPost: any) => {
    const cleaned = removeOldWPTOC(rawPost.content);
    ...
    setToc(tocItems);
    setPost({ ...rawPost, content: updatedHtml });
  };

  useEffect(() => {
    if (initialPost) {
      applyPostContent(initialPost);
